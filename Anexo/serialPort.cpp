...
// Inicio Construtores
	...
	serialPort::serialPort(): io(), baud(57600), bitNumber(8), parity(boost::asio::serial_port_base::parity::none), stopBits(boost::asio::serial_port_base::stop_bits::one), port(io,"/dev/ttyUSB0")
 	{
		port.set_option( baud );
		port.set_option( bitNumber );
		port.set_option( parity );
